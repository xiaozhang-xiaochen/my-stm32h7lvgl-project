 ...
 * 第一次发布
 * V1.1 20230607
 * 修改SYS_SUPPORT_OS宏逻辑, 包含头文件改为:"os.h"
 * 删除USART_UX_IRQHandler()函数中的超时逻辑,修改HAL_UART_RxCpltCallback()
 *
 ****************************************************************************************************
 */
...
__asm(".global __ARM_use_no_argv \n\t");    /* AC6下需要定义main函数为无参数形式以避免部分库函数报错 */

#else
/* 使用AC5编译器时, 需在这里定义__FILE 结构体以支持不使用半主机模式 */
...
/* 不使用半主机模式，至少需要重定义_ttywrch\_sys_exit\_sys_command_string函数,以兼容AC6和AC5模式 */
int _ttywrch(int ch)
{
    ch = ch;
    return ch;
 ...
 * @note        注意: 必须确保有正确的时钟源, 否则串口波特率显示会出现异常.
 *              常用的USART的时钟源在sys_stm32_clock_init()函数中已经配置好了.
 * @retval      无
 */
