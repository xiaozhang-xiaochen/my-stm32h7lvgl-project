...
void sdram_init(void)
{
    ...
    sdram_timing.LoadToActiveDelay = 2;                                       /* 加载模式寄存器到激活状态的延时为2个时钟周期 */
    sdram_timing.ExitSelfRefreshDelay = 8;                                    /* 退出自刷新延时为8个时钟周期 */
    sdram_timing.SelfRefreshTime = 7;                                         /* 自刷新时间为7个时钟周期 */
    sdram_timing.RowCycleDelay = 7;                                           /* 行循环延时为7个时钟周期 */
    sdram_timing.WriteRecoveryTime = 2;                                       /* 写恢复延时为2个时钟周期 */
    sdram_timing.RPDelay = 2;                                                 /* 行预充电延时为2个时钟周期 */
    sdram_timing.RCDDelay = 2;                                                /* 行到列的延时为2个时钟周期 */
...
void sdram_initialization_sequence(void)
{
    ...
    /* SDRAM控制器初始化完成以后,需要按照如下顺序初始化SDRAM */
     ...
     * bit9为指定写突发模式，bit10和bit11位保留位 */
    temp = (uint32_t)SDRAM_MODEREG_BURST_LENGTH_1  |                  /* 设置突发长度:1(可选1/2/4/8) */
              SDRAM_MODEREG_BURST_TYPE_SEQUENTIAL  |                  /* 设置突发类型:连续(可选连续/间断) */
              SDRAM_MODEREG_CAS_LATENCY_2          |                  /* 设置CAS值:2(可选2/3) */
              SDRAM_MODEREG_OPERATING_MODE_STANDARD|                  /* 设置操作模式:0,标准模式 */
              SDRAM_MODEREG_WRITEBURST_MODE_SINGLE;                   /* 设置突发写模式:1,单点写 */
    sdram_send_cmd(0, FMC_SDRAM_CMD_LOAD_MODE, 1, temp);              /* 设置SDRAM的模式寄存器 */
}

/**
 * @brief       SDRAM底层驱动，引脚配置，时使能
...
void HAL_SDRAM_MspInit(SDRAM_HandleTypeDef *hsdram)
{
    ...
    gpio_init_struct.Alternate = GPIO_AF12_FMC;  /* 设置为FMC */
...
/**
 * @brief       在指定地址((WriteAddr+BANK5_SDRAM_ADDR))开始,连续读取n个字节.
 * @param       pbuf     : 字节指针
 * @param       readaddr : 要读取的开始地址
 * @param       n        : 要读取的字节数
 * @retval      无
*/
